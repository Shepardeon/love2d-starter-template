...
function debug.update()
    ...
    debug.stats.avgUpdateTime = utils.avgTable(lume.map(debug.dataPoints, function(v)
        return v.updateTime
    end))
    debug.stats.avgDrawTime = utils.avgTable(lume.map(debug.dataPoints, function(v)
        return v.drawTime
    end))
    ...
    debug.info = {
        ...
        'Draw Time: ' .. lume.round(debug.stats.avgDrawTime, 0.001) .. 'ms (' .. lume.round(
            debug.stats.avgDrawTimePercent,
            0.1
        ) .. '%)',
        'Update Time: ' .. lume.round(debug.stats.avgUpdateTime, 0.001) .. 'ms (' .. lume.round(
            debug.stats.avgUpdateTimePercent,
            0.1
        ) .. '%)',
        'RAM: ' .. lume.round(ram, 0.01) .. ramUnit,
        'VRAM: ' .. lume.round(vram, 0.01) .. vramUnit,
...
function debug.drawGraph(x, y, barScale, barWidth)
    ...
    for i, v in ipairs(debug.dataPoints) do
        ...
        love.graphics.rectangle('fill', x + (i * barWidth), y + barSize, barWidth, -barSize)

        love.graphics.setColor(0, 1, 0, 1)
        love.graphics.rectangle('fill', x + (i * barWidth), y + barSize, barWidth, -v.updateTime / frameTime * barSize)
...
function debug.draw(x, y, barScale, barWidth)
    ...
    if not debug.config.drawDebug then
        return
    end
